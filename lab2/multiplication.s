...
l2_notaut:
	...
	pushl %eax			#przechowanie wartosci rejestru eax
	imul %ebx			#odejmowanie od siebie liczb
	...
	inc %ecx
	add %edi,%ecx
	add %esi,%ecx			#indeks pierwszego elementu
	clc
		
	movl wynik(,%ecx,4), %ebx	#pierwszy element wyniku do dodania z mnozenia
	add %eax, %ebx
	movl %ebx, wynik(,%ecx,4)
	dec %ecx
	movl wynik(,%ecx,4), %ebx	#drugi element wyniku do dodania z mnozenia
	add %edx, %ebx
	movl %ebx, wynik(,%ecx,4)
	dec %ecx
addition_carry:
	movl wynik(,%ecx,4), %ebx
	add $0, %ebx			#dodaj przeniesienie
	movl %ebx, wynik(,%ecx,4)
	dec %ecx
	cmpl $0, %ecx			
	jne addition_carry		#dopuki nie dojdzie do konca wyniku
	
	popf
	popl %eax			#przywrocenie wartosci rejestru eax
	pushf
	cmpl $0, %esi			#czy indeks l2 jest równy 0
	jne multiplication_inner	#jeżeli nie to pomnóż przez kolejną porcje liczby 2
	cmpl $0, %edi			#czy indeks l1 jest 0
	jne multiplication		#jezeli nie to wroc
