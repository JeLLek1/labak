#mnożenie wektorów a=a*b sekwencyjnie
#
#argumenty funckji
...
mulSISD:
	...
	fmul bpos0(%ebp) 		#mnożenie wartości wektora B 0 pozycju
	fstp (%eax, %ecx, 4) 		#zwrocenie wyniku do wektora A 0 pozycji
	incl %ecx			#zwiekszenie indeksu

	fld (%eax, %ecx, 4)		#adres pamięci wetkra A do rejestru
	fmul bpos1(%ebp) 		#mnożenie wartości wektora B 1 pozycju
	fstp (%eax, %ecx, 4) 		#zwrocenie wyniku do wektora A 1 pozycji
	incl %ecx			#zwiekszenie indeksu

	fld (%eax, %ecx, 4)		#adres pamięci wetkra A do rejestru
	fmul bpos2(%ebp) 		#mnożenie wartości wektora B 2 pozycju
	fstp (%eax, %ecx, 4) 		#zwrocenie wyniku do wektora A 2 pozycji
	incl %ecx			#zwiekszenie indeksu

	fld (%eax, %ecx, 4)		#adres pamięci wetkra A do rejestru
	fmul bpos3(%ebp) 		#mnożenie wartości wektora B 3 pozycju
